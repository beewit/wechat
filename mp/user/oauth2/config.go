...
const (
	AUTHORIZE_CODE = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=%s&redirect_uri=%s&response_type=code&scope=%s&state=STATE#wechat_redirect"
)

func (cfg *OAuth2Config) GetAuthorizeCodeUrl(redirectUri string) string {
	return fmt.Sprintf(AUTHORIZE_CODE, cfg.AppId, redirectUri, "snsapi_userinfo")
}
