...
//  SceneId:       场景值ID, 为32位非0整型
//  ExpireSeconds: 二维码有效时间, 以秒为单位.  最大不超过 604800.
func (clt *Client) CreateTemporaryQRCodeSceneStr(SceneStr string, ExpireSeconds int) (qrcode *TemporaryQRCode, err error) {
	if SceneStr == "" {
		err = errors.New("SceneId should be greater than 0")
		return
	}
	...
	result.TemporaryQRCode.SceneString = SceneStr
	qrcode = &result.TemporaryQRCode
	return
}
